...
```shell-session
LeDaav@htb[/htb]$ which wireshark
```
...
```shell-session
LeDaav@htb[/htb]$ sudo apt install wireshark 
```
...
```shell-session
LeDaav@htb[/htb]$ which tshark

LeDaav@htb[/htb]$ tshark -D

LeDaav@htb[/htb]$ tshark -i 1 -w /tmp/test.pcap
...
```shell-session
LeDaav@htb[/htb]$ sudo tshark -i eth0 -w /tmp/test.pcap
```
...
```shell-session
LeDaav@htb[/htb]$ sudo tshark -i eth0 -f "host 172.16.146.2"
