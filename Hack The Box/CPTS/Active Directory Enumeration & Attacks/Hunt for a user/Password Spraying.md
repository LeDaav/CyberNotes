...
```shell-session
LeDaav@htb[/htb]$ enum4linux -U 172.16.5.5  | grep "user:" | cut -f2 -d"[" | cut -f1 -d"]"
...
```shell-session
LeDaav@htb[/htb]$ rpcclient -U "" -N 172.16.5.5
...
```shell-session
LeDaav@htb[/htb]$ crackmapexec smb 172.16.5.5 --users
...
```shell-session
LeDaav@htb[/htb]$ ldapsearch -h 172.16.5.5 -x -b "DC=INLANEFREIGHT,DC=LOCAL" -s sub "(&(objectclass=user))"  | grep sAMAccountName: | cut -f2 -d" "
...
```shell-session
LeDaav@htb[/htb]$ ./windapsearch.py --dc-ip 172.16.5.5 -u "" -U
...
```shell-session
LeDaav@htb[/htb]$  kerbrute userenum -d inlanefreight.local --dc 172.16.5.5 /opt/jsmith.txt 
...
```shell-session
LeDaav@htb[/htb]$ sudo crackmapexec smb 172.16.5.5 -u htb-student -p Academy_student_AD! --users
