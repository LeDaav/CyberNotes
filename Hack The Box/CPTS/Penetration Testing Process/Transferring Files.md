...
```shell-session
LeDaav@htb[/htb]$ cd /tmp
LeDaav@htb[/htb]$ python3 -m http.server 8000
...
```shell-session
LeDaav@htb[/htb]$ scp linenum.sh user@remotehost:/tmp/linenum.sh
...
```shell-session
LeDaav@htb[/htb]$ base64 shell -w 0
