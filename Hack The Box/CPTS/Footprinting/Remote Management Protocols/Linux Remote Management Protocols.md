...
#### Default Configuration
        ...
        shellsession
`LeDaav@htb[/htb]$ cat /etc/ssh/sshd_config  | grep -v "#" | sed -r '/^\s*$/d' Include /etc/ssh/sshd_config.d/*.conf ChallengeResponseAuthentication no UsePAM yes X11Forwarding yes PrintMotd no AcceptEnv LANG LC_* Subsystem       sftp    /usr/lib/openssh/sftp-server`
...
We can next probe the service a bit to see what we can gain access to.
        ...
        shellsession
`LeDaav@htb[/htb]$ nc -nv 127.0.0.1 873 (UNKNOWN) [127.0.0.1] 873 (rsync) open @RSYNCD: 31.0 @RSYNCD: 31.0 #list dev             Dev Tools @RSYNCD: EXIT`
...
#### /etc/hosts.equiv
        ...
        shellsession
`LeDaav@htb[/htb]$ cat /etc/hosts.equiv # <hostname> <local username> pwnbox cry0l1t3`
...
#### Sample .rhosts File
        ...
        shellsession
`LeDaav@htb[/htb]$ cat .rhosts htb-student     10.0.17.5 +               10.0.17.10 +               +`
...
#### Logging in Using Rlogin
        ...
        shellsession
`LeDaav@htb[/htb]$ rlogin 10.0.17.2 -l htb-student Last login: Fri Dec  2 16:11:21 from localhost [htb-student@localhost ~]$`
...
#### Listing Authenticated Users Using Rwho
        ...
        shellsession
`LeDaav@htb[/htb]$ rwho root     web01:pts/0 Dec  2 21:34 htb-student     workstn01:tty1  Dec  2 19:57  2:25`
...
To provide additional information in conjunction with `rwho`, we can issue the `rusers` command. This will give us a more detailed account of all logged-in users over the network, including information such as the username, hostname of the accessed machine, TTY that the user is logged in to, the date and time the user logged in, the amount of time since the user typed on the keyboard, and the remote host they logged in from (if applicable).
        ...
        shellsession
`LeDaav@htb[/htb]$ rusers -al 10.0.17.5 htb-student     10.0.17.5:console          Dec 2 19:57     2:25`
