...
```shell-session
LeDaav@htb[/htb]$ sudo apt install vsftpd 
```
...
```shell-session
LeDaav@htb[/htb]$ cat /etc/vsftpd.conf | grep -v "#"
```
...
```shell-session
LeDaav@htb[/htb]$ cat /etc/ftpusers
...
```shell-session
LeDaav@htb[/htb]$ ftp 10.129.14.136
...
```shell-session
LeDaav@htb[/htb]$ ls | grep Notes.txt
...
```shell-session
LeDaav@htb[/htb]$ wget -m --no-passive ftp://anonymous:anonymous@10.129.14.136
...
```shell-session
LeDaav@htb[/htb]$ touch testupload.txt
```
...
```shell-session
LeDaav@htb[/htb]$ sudo nmap --script-updatedb
...
```shell-session
LeDaav@htb[/htb]$ find / -type f -name ftp* 2>/dev/null | grep scripts
...
```shell-session
LeDaav@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136
...
```shell-session
LeDaav@htb[/htb]$ sudo nmap -sV -p21 -sC -A 10.129.14.136 --script-trace
...
```shell-session
LeDaav@htb[/htb]$ nc -nv 10.129.14.136 21
```

  FTP

```shell-session
LeDaav@htb[/htb]$ telnet 10.129.14.136 21
```
...
```shell-session
LeDaav@htb[/htb]$ openssl s_client -connect 10.129.14.136:21 -starttls ftp
