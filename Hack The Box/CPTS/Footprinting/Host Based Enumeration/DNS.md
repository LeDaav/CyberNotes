...
![Diagram showing domain hierarchy: Root, Top Level Domains (TLD) like net, org, com, dev, io; Second Level Domain inlanefreight.com; Sub-Domains dev.inlanefreight.com, www.inlanefreight.com, mail.inlanefreight.com; Host WS01.dev.inlanefreight.com.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/27/tooldev-dns.png)
...
```shell-session
LeDaav@htb[/htb]$ dig soa www.inlanefreight.com
...
```shell-session
LeDaav@htb[/htb]$ dig ns inlanefreight.htb @10.129.14.128
...
```shell-session
LeDaav@htb[/htb]$ dig CH TXT version.bind 10.129.120.85
...
```shell-session
LeDaav@htb[/htb]$ dig any inlanefreight.htb @10.129.14.128
...
```shell-session
LeDaav@htb[/htb]$ dig axfr inlanefreight.htb @10.129.14.128
...
```shell-session
LeDaav@htb[/htb]$ dig axfr internal.inlanefreight.htb @10.129.14.128
...
```shell-session
LeDaav@htb[/htb]$ for sub in $(cat /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.14.128 | grep -v ';\|SOA' | sed -r '/^\s*$/d' | grep $sub | tee -a subdomains.txt;done
...
```shell-session
LeDaav@htb[/htb]$ dnsenum --dnsserver 10.129.14.128 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb
