...
```shell-session
LeDaav@htb[/htb]$ cat /etc/apt/sources.list.d/parrot.list
...
```shell-session
LeDaav@htb[/htb]$ apt-cache search impacket
...
```shell-session
LeDaav@htb[/htb]$ apt-cache show impacket-scripts
...
```shell-session
LeDaav@htb[/htb]$ apt list --installed
...
```shell-session
LeDaav@htb[/htb]$ sudo apt install impacket-scripts -y
...
```shell-session
LeDaav@htb[/htb]$ mkdir ~/nishang/ && git clone https://github.com/samratashok/nishang.git ~/nishang
...
```shell-session
LeDaav@htb[/htb]$ wget http://archive.ubuntu.com/ubuntu/pool/main/s/strace/strace_4.21-1ubuntu1_amd64.deb
...
```shell-session
LeDaav@htb[/htb]$ sudo dpkg -i strace_4.21-1ubuntu1_amd64.deb 
