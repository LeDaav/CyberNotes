...
```shell-session
LeDaav@htb[/htb]$ sudo apt-get install apache2
```
...
```shell-session
LeDaav@htb[/htb]$ chmod 700 filename
```
...
```shell-session
LeDaav@htb[/htb]$ mount -F nfs 10.129.15.122:/nfs_share /mnt/local
```
...
```shell-session
LeDaav@htb[/htb]$ sudo lsof -c apache2
```
...
```shell-session
LeDaav@htb[/htb]$ sudo strace -p `pgrep apache2`
```
