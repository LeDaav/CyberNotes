...
```shell-session
LeDaav@htb[/htb]$ smbclient -L SERVER_IP -U htb-student
...
```shell-session
LeDaav@htb[/htb]$ smbclient '\\SERVER_IP\Company Data' -U htb-student
...
```shell-session
LeDaav@htb[/htb]$ sudo mount -t cifs -o username=htb-student,password=Academy_WinFun! //ipaddoftarget/"Company Data" /home/user/Desktop/
```
...
```shell-session
LeDaav@htb[/htb]$ sudo apt-get install cifs-utils
```
